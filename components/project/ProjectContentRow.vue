  ...
  <tr class="collapse show"
    ...
    <td class="px-0" v-if="edit">
      <div class="d-flex flex-row">
        <button type="button" class="btn btn-danger py-0 px-2" @click="markDelete()"><i class="bi bi-trash"></i></button>
        <button type="button" class="btn btn-secondary py-0 px-2 mx-2" @click="moveUp()"><i class="bi bi-arrow-bar-up"></i></button>
        <button type="button" class="btn btn-secondary py-0 px-2 mx-2" @click="moveDown()"><i class="bi bi-arrow-bar-down"></i></button>
      </div>
    </td>
    <td class="content-name" data-bs-toggle="collapse" :data-bs-target="'.parent-id-' + projectContent.id" nowrap>{{ projectContent.item.itemName }}</td>
    ...
    <td v-if="projectContent.auto" nowrap>{{ getBuild }}</td>
    <td v-if="projectContent.auto" nowrap>{{ getPlanned }}</td>
      ...
      getCostAmount(itemID) {
        if (this.projectContent.item.blueprint) {
          for (let cost of this.projectContent.item.blueprint.baseCost) {
            if (cost.item == itemID) {
              return (cost.quantity / 1.5 * this.projectContent.efficiency * this.getPlanned).toFixed(0);
              break;
            }
          }
        }
